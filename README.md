...
内部検証担当は実装担当内の読み取り専用サブエージェントであり、独立セッションや正式監査ではない。監査数にも含めない。
...
- **実装担当は既存パターンを調査し、Builderと別コンテキストの内部検証担当で制限付きループを行う。** 内部検証は正式監査ではない。既存と異なるリファクタが必要なら実装前に提案し、勝手に混ぜない
