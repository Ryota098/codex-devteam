...
PMが準備 ──▶ Builder↔内部検証 ──▶ PMが裏取り ──▶ 基本2監査 ──▶ PMが整理
...
内部検証担当は実装担当内の読み取り専用サブエージェントであり、コード等の変更時はユーザーの追加指示なしで自動起動する。独立セッションや正式監査ではなく、監査数にも含めない。起動できなければ自己確認で代替せず停止する。
...
実装 : （Builderが実装・テスト → 内部検証担当が別コンテキストで確認
        → 修正必要ならBuilderへ返す制限付きループ）合格しました。コミットをお願いします。
...
| 7 | 実装 | ★「承認します。実装に進んでください」 | Builder↔内部検証担当 → loop-state.md → コミット依頼（あなたが実行） → report.md / summary.md |
...
- **内部検証担当を毎回指示する必要がある？** → 不要。コード・テスト・設定等の変更では実装担当が自動起動し、合格前はコミット依頼やPM提出へ進まない
...
- **実装担当は既存パターンを調査し、Builderと自動起動する別コンテキストの内部検証担当で制限付きループを行う。** 内部検証は正式監査ではない。既存と異なるリファクタが必要なら実装前に提案し、勝手に混ぜない
